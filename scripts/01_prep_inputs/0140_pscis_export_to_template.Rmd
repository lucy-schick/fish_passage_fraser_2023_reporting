...
Fraser 2024 - no sites have this issue.
...
- append MoTi ids to the comments - for Fraser 2024 we only have the one `moti_chris_culvert_id` column and not `moti_chris_culvert_id2` or `moti_chris_culvert_id3` so we will comment out the code to add the 2nd and 3rd culvert ids. 
...
pscis_export <- pscis_export_raw |>
  ...
  dplyr::mutate(assessment_comment = dplyr::case_when(
    ...
    TRUE ~ assessment_comment)) |>
  #for Fraser 2024 we only have the one `moti_chris_culvert_id` column so lets comment out these below
...
pscis_export |> 
  readr::write_csv('data/inputs_extracted/pscis_export_submission_2024.csv', na='')
