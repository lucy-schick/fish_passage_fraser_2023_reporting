...
An aerial survey was conducted with a remotely piloted aircraft and the resulting imagery was processed into an orthomosaic, digital terrain model (dtm) and a digital surface model (dsm), available to view and download `r if(gitbook_on){knitr::asis_output("in Table \\@ref(tab:tab-uav-imagery-cap-199171).")}else(knitr::asis_output(ngr::ngr_str_link_url(url_base = params$report_url, anchor_text = "here")))`

<br>

```{r tab-uav-imagery-cap-199171, results="asis", eval= gitbook_on}
my_caption <- paste0("Drone imagery download and viewer links for crossings ", my_site, " and ", my_site2, ".")

my_tab_caption()
```

```{r tab-uav-199171, eval= gitbook_on}
site_item <- c("199171_burnt_cabin_gala")

project_uav |> 
  dplyr::filter(item %in% site_item) |> 
